...
public class MLFeed {
    ...
    public void getHome() {
        try {
            enableStrictMode();
            Trends trending = tweet.getPlaceTrends(1);
            ArrayList<Interest> omoshiro = new ArrayList<Interest>();

            //for(Trend trend: trending.getTrends()) {

                //if(!(trend.getName().indexOf('?') >= 0)) {
                //System.out.println("trending: "+ trend.getName());
                //omoshiro.add(new Interest(trend.getName()));
                //}
                //addPosts(omoshiro);

            //}
            Trend[] t = trending.getTrends();
            for(int i = 0; i< 10; i++){
                omoshiro.add(new Interest(t[i].getName()));
            }

            addPosts(omoshiro);
        } catch (TwitterException e) {
            // TODO Auto-generated catch block
            System.out.println("--------not able to get trends----------------");
            e.printStackTrace();
        }
    }
    ...
    public void addPosts(ArrayList<Interest> interests) {
        ...
        for(Interest inter: interests) {
            ...
            int fav;
            for(Status stats: feed) {
                rt = stats.getFavoriteCount();
                fav = stats.getRetweetCount();
                post = new Posts("https://twitter.com/" + stats.getUser().getScreenName()+ "/status/" + stats.getId(), stats.getFavoriteCount(), stats.getRetweetCount() );
                hashs = stats.getHashtagEntities();
                for(HashtagEntity hash: hashs) {
                    post.addKeyWords(hash.getText());
                }
    ...
    public ArrayList<Status> searchTweets(String str) throws TwitterException {
        ...
        for(Status stats: qr.getTweets()) {
            ave = (stats.getFavoriteCount() + stats.getRetweetCount())/2;
            if( ave > 50)
                tweets.add(stats);
